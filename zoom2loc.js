...
zoom2loc = function (event) {
    ...
    const { width: navigatorImageWidth, height: navigatorImageHeight } = document.getElementById("cvBase")
    const { width: slideWidth, height: slideHeight } = tilmap.viewer.world.getItemAt(0).source
    console.log(clickPos.x, clickPos.y, navigatorImageWidth, navigatorImageHeight, slideWidth, slideHeight)

    tilmap.viewer.viewport.zoomTo(tilmap.viewer.viewport.getMaxZoom() < 100 ? tilmap.viewer.viewport.getMaxZoom()/3 : tilmap.viewer.viewport.getMaxZoom()/4 )
    tilmap.viewer.viewport.panTo(tilmap.viewer.viewport.imageToViewportCoordinates(clickPos.x*slideWidth/navigatorImageWidth, clickPos.y*slideHeight/navigatorImageHeight))
